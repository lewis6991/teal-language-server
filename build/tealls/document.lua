...
function Document:get_type_info_from_id(id)
   local tr = self:get_type_report()
   if not tr then
      return
   end

   local info = tr.types[id]

   if info and info.ref then
      return self:get_type_info_from_id(info.ref)
   end
...
function Document:get_type_info(tk)
   ...
   local info = self:get_type_info_from_id(type_id)
   if info then
      return info
   end
...
function Document:symbols_in_scope_at(where)
   local tr = self:get_type_report()
   if not tr then
      return
   end
